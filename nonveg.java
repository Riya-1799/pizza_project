...
public class nonveg extends pizza_menu {
	...
	void veg_display() {
		...
		if(topping1.equals("onion")) {
		 total = p.add(non_veg, onion);
		}
		if(topping1.equals("tomatoes")) {
			total = p.add(non_veg, tomatoes);
		}
		if(topping1.equals("mushroom")) {
			total = p.add(non_veg, mushroom);
		}
		if(topping1.equals("jalaphno")) {
			total = p.add(non_veg, jalaphno);
		
		}	
		if(topping1.equals("corn")) {
			total = p.add(non_veg, corn);
